...
- Derives line items a filer doesn't directly tag via backfill logic — see `_backfill_income_statement()` (Gross Profit, Cost Of Revenue when only Gross Profit is tagged, Operating Income, Operating Expense), `_backfill_balance_sheet()` (Total Debt), `_backfill_cash_flow()` (Free Cash Flow)
...
- When statement numbers look inconsistent (a metric stuck on a stale date, a reconciliation identity like Revenue − Cost = Gross Profit failing), the fix is almost always one of the two mechanisms above — check whether a tag got retired/switched, or whether the filer reports cumulative YTD instead of discrete quarters, before assuming it's a one-off ticker quirk
